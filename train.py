...
def validation_run(config, model, validation_dataloader, epoch: int):
    ...
    with torch.no_grad():
        ...
        for valid_batch in validation_dataloader:
            ...
            valid_loss = loss_function(logits_valid, valid_batch["label"].to(config["device"]).float().unsqueeze(-1), prompts = valid_batch["prompt"])
...
def train_one_epoch(config, model, train_dataloader, loss_function, optimizer):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        loss = loss_function(logits, batch["label"].to(config["device"]).float().unsqueeze(-1), prompts = batch["prompt"])
